...
Example 2: Using major isoforms in one cell type
...
can't subset like I was doing for igraph, need to go into the adjacency matrix and
replace with zeros ...
...
pdf(paste0(fig_dir, 'test_ggnet.pdf'), width=5, height=5)
p
dev.off()
...
test <- rgraph(10, tprob = 0.2)

hubs <- GetHubGenes(seurat_obj)

n <- network(TOM[hubs$gene_name, hubs$gene_name], directed = FALSE)

n %v% "module" <- as.character(hubs$module)
n %v% "kME" <- hubs$kME
...
p <- ggplot(n, aes(x = x, y = y, xend = xend, yend = yend)) +
  ...
  geom_nodes(aes(color = module, size = kME)) +
  theme_blank()

pdf(paste0(fig_dir, 'test_ggnet2.pdf'), width=5, height=5)
p
dev.off()
